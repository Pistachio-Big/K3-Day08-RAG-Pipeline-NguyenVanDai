...
def retrieve(
    ...
    # Step 2: Merge bằng RRF (giữ điểm cosine gốc trong dense_results để dùng ở step 4)
    merged = rerank_rrf([dense_results, sparse_results], top_k=top_k * 2)
    for item in merged:
        item["source"] = "hybrid"

    # Step 3: Rerank
    # Khi RERANK_METHOD == "rrf", merge ở step 2 đã chính là bước "rerank" (fusion
    # theo rank) — gọi lại rerank(..., method="rrf") sẽ raise NotImplementedError vì
    # interface đó yêu cầu gọi rerank_rrf trực tiếp với nhiều ranked lists, không phải
    # qua unified rerank(). Chỉ gọi rerank() khi dùng phương pháp khác (cross_encoder/mmr).
